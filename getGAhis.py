...
def main():
    ...
    while t1 < '2020-01-01':
                ...
                'reportRequests': [
                    {
                        ...
                        'dimensions': [{'name': 'ga:sourceMedium'}, {'name': 'ga:city'}, {'name': 'ga:operatingSystem'}, {'name': 'ga:appVersion'}],
        ...
        sql = 'insert into ga_his_fivedimension(date,mass,source,med,city,device,edition,flowNum,userNum,sessionSeconds,terraceId,viewId,dateType) values '
        for report in response.get('reports', []):
            for row in report.get('data', {}).get('rows', []):
                ...
                sql = sql + '("%s","%s","%s","%s","%s","%s","%s",%s,%s,%s,%d,%s,"%s"),' % (t1+"~"+t2, dimensions[0], source, medium, dimensions[1], dimensions[2], dimensions[3],
