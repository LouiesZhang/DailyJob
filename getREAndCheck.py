...
def to_mysql(df1, RorE):
    ...
    if RorE == 0:
        df1.to_sql('mass_register_all', engine, index=False, if_exists='append')
    else:
        df1.to_sql('mass_entry_all', engine, index=False, if_exists='append')


# 核对注册
def check_re(analytics, conn, cursor, terraceId, viewId):
    sql = 'select * from mass_register_all a where a.terraceId = "%d" and a.ifCheck is null order by a.registerDate' % (terraceId)
    registerResult = None
    try:
        cursor.execute(sql)
    ...
    for index, rRow in enumerate(registerResult):
        if index == 0:
            ...
            if reDate == rRow[1].strftime('%Y-%m-%d') and index != len(registerResult)-1:
                oneDayRows.append(rRow)
            else:
                if index == len(registerResult)-1 and reDate == rRow[1].strftime('%Y-%m-%d'):
                    oneDayRows.append(rRow)
                print(reDate)
                if terraceId == 1:
                    #第一种方式
                    ...
                    response = GaGet.get_user_message_by_account_igold(analytics, viewId, reDate, accounts[:-1])
                    ...
                    for rr in oneDayRows:
                        gaIds = gaIds + rr[9] + "|"
                    response = GaGet.get_user_message_igold(analytics, viewId, reDate, gaIds[:-1])
                    data = response.get('reports', [])[0].get('data', {})
                    if isinstance(data.get('rowCount'), int) is True:
                        rowDatas = data.get('rows', [])
                        deleteList = []
                    ...
                    for rr in oneDayRows:
                        gaIds = gaIds + rr[9] + "|"
                    response = GaGet.get_user_message(analytics, viewId, reDate, gaIds[:-1])
                    data = response.get('reports', [])[0].get('data', {})
                    if isinstance(data.get('rowCount'), int) is True:
                        rowDatas = data.get('rows', [])
                        deleteList = []
                    ...
                    for rr in oneDayRows:
                        accounts = accounts + rr[10] + "|"
                    response = GaGet.get_user_message_by_account(analytics, viewId, reDate, accounts[:-1])
                    data = response.get('reports', [])[0].get('data', {})
                    if isinstance(data.get('rowCount'), int) is True:
                        rowDatas = data.get('rows', [])
                        deleteList = []
                    ...
                    if len(oneDayRows) > 0:
                        ...
                        updateSql = "UPDATE mass_register_all SET ifCheck = 1 where id in (%s)" % (checkIds[:-1])
                        try:
                            cursor.execute(updateSql)
                            conn.commit()
                        except:
                    ...
                    for rr in oneDayRows:
                        accounts = accounts + rr[10] + "|"
                    response = GaGet.get_user_message_by_account_acetop(analytics, viewId, reDate, accounts[:-1])
                    data = response.get('reports', [])[0].get('data', {})
                    if isinstance(data.get('rowCount'), int) is True:
                        rowDatas = data.get('rows', [])
                        deleteList = []
        ...
        # # 如果确实查不到，到库中修改为已核对
        # if isinstance(data.get('rowCount'), int) is False:
        #     updateSql = "UPDATE mass_register_all SET ifCheck = 1 where id = '%d'" % (rRow[0])
        #     try:
        #         cursor.execute(updateSql)
        #         conn.commit()
        #     except:
...
def check_en(analytics, conn, cursor, terraceId, viewId):
    sql = 'select * from mass_entry_all a where a.terraceId = "%d" and a.ifCheck is null' % (terraceId)
    entryResult = None
    try:
        cursor.execute(sql)
    ...
    for eRow in entryResult:
        ...
        #如果确实查不到，到库中修改为已核对
        if isinstance(data.get('rowCount'), int) is False:
            updateSql = "UPDATE mass_entry_all SET ifCheck = 1 where id = '%d'" % (eRow[0])
            try:
                cursor.execute(updateSql)
                conn.commit()
            except:
        ...
        if len(checkData) > 1:
            source = checkData[1].split('/')[0].rstrip()
            medium = checkData[1].split('/')[1].lstrip()
            updateSql = "UPDATE mass_entry_all SET extensionSource = '%s', source = '%s', medium = '%s', plan = '%s'," \
...
def main():
    ...
    cursor = conn.cursor()
    sql = 'select * from mass_terrace where id = 4'
    result = None
    try:
        cursor.execute(sql)
