...
def getMass_report(analytics, startDate, endDate, viewId):
...
def get_user_message_by_account_igold(analytics, viewId, rDate, account):
  ...
  if (realDate >= '2019-09-07'):
      url = 'member.igoldhk.club/opentrueaccount/RegistrationSuccess'
  else:
      url = 'nickName'
  return analytics.reports().batchGet(
      body={
          'reportRequests': [
              {
                  ...
                  'dimensions': [{'name': 'ga:searchDestinationPage'}, {'name': 'ga:sourceMedium'}, {'name': 'ga:campaign'}, {'name': 'ga:adContent'}, {'name': 'ga:keyword'}],
                  'dimensionFilterClauses': [
                      {
                          'operator': 'AND',
                          'filters': [
...
def get_user_message_by_account_acetop(analytics, viewId, rDate, account):
  realDate = datetime.datetime.strptime(rDate + '', '%Y-%m-%d').strftime('%Y-%m-%d')
  return analytics.reports().batchGet(
      body={
          'reportRequests': [
